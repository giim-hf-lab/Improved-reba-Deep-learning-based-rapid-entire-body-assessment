...
def evaluate(test_generator, action=None, return_predictions=False, use_trajectory_model=False):
    ...
    with torch.no_grad():
        ...
        for _, batch, batch_2d in test_generator.next_epoch():
            ...
            # Positional model
            if not use_trajectory_model:
                predicted_3d_pos = model_pos(inputs_2d)   #predicted_3d_pos.size()=([2, 726, 17, 3])
            else:
            ...
            if test_generator.augment_enabled():
                ...
                # print('1@@@@@predicted_3d_pos', predicted_3d_pos.size())
            if return_predictions:
                return predicted_3d_pos.squeeze(0).cpu().numpy()   ##return   shape=(726, 17, 3)
...
if args.render:
    ...
    gen = UnchunkedGenerator(None, None, [input_keypoints],
                             ...
                             kps_left=kps_left, kps_right=kps_right, joints_left=joints_left, joints_right=joints_right)
    prediction = evaluate(gen, return_predictions=True)   #(726, 17, 3)  726帧，17个关节点，3为坐标，根节点间相对坐标系
    ...
    if args.viz_export is not None:
        ...
        # np.save(args.viz_export, prediction)         #@@@######have been revised
    ...
    if args.viz_output is not None:
        ...
        if ground_truth is not None:
            ...
            np.save(args.viz_export, prediction)  #               ########@@@@##have been addded

        anim_output = {'Reconstruction': prediction}  #(726,17,3)
        if ground_truth is not None and not args.viz_no_ground_truth:
            anim_output['Ground truth'] = ground_truth
        
        input_keypoints = image_coordinates(input_keypoints[..., :2], w=cam['res_w'], h=cam['res_h'])
        #input_keypoints.shape =（726,17,2）
